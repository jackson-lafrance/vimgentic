...
local function project_fixture(callback, options)
  ...
  local original_select, original_notify = select_ui.open, util.notify
  select_ui.open = function(items, picker_options, on_choice)
  ...
  cleanup(sidebar)
  select_ui.open, util.notify = original_select, original_notify
...
describe("vimgentic.chat.terminal continuity", function()
  ...
  it("a failed new-chat stop releases the action without leaving a delayed restart", function()
    local fail = true
    project_fixture(function(state)
      state.sidebar:open()
      state.sidebar:new_chat()
      state.choices[1].choose("Start new chat")
      truthy(state.notices[1]:find("stop failed", 1, true))
      truthy(state.sidebar:open())
      state.exits[1](0)
      eq(nil, state.starts[2])
      eq("/tmp/project", state.starts[1].cwd)
    end, { stop_job = function()
      if fail then fail = false; error("stop failed") end
    end })
  end)

  it("a failed history stop reports the error and allows reopening the original chat", function()
    local fail = true
    project_fixture(function(state)
      state.sidebar:open()
      local called, switched = pcall(state.sidebar.switch_session, state.sidebar, "/saved.jsonl", "/other-project")
      eq(true, called)
      eq(false, switched)
      truthy(state.notices[1]:find("stop failed", 1, true))
      truthy(state.sidebar:open())
  ...
  it("a failed restart hook releases the pending action after the old process exits", function()
    local starts = 0
    project_fixture(function(state)
      state.sidebar:open()
      state.sidebar:new_chat()
      state.choices[1].choose("Start new chat")
      local exited = pcall(state.exits[1], 0)
      eq(true, exited)
      truthy(state.notices[1]:find("restart hook failed", 1, true))
      truthy(state.sidebar:open())
      eq("/tmp/project", state.starts[2].cwd)
      eq(nil, state.starts[3])
    end, { defer_exit = true, on_start = function()
      starts = starts + 1
      if starts == 2 then error("restart hook failed") end
    end })
  end)
  ...
  it("a directory change while the confirmation is open cancels the stale action", function()
    project_fixture(function(state)
      state.sidebar:open()
      state.chdir("/tmp/other-project")
      state.sidebar:send_text("inspect the other project")
  ...
  it("opening a hidden chat after a directory change reuses the process without a menu", function()
    project_fixture(function(state)
      state.sidebar:open()
      local buffer = state.sidebar.buffer
      state.sidebar:close()
      state.chdir("/tmp/other-project")
      truthy(state.sidebar:open())
  ...
  it("cancelling a new chat preserves the old process and allows reopening", function()
    project_fixture(function(state)
      state.sidebar:open()
      truthy(state.sidebar:new_chat())
      eq({}, state.stops)
      eq({ "Start new chat", "Cancel" }, state.choices[1].items)
      truthy(state.choices[1].options.prompt:find("discards unsent input", 1, true))
      state.choices[1].choose(nil)
      state.sidebar:close()
      truthy(state.sidebar:open())
      eq(1, #state.starts)
  ...
  it("confirming a new chat waits for the old process and uses a fresh session in the editor directory", function()
    project_fixture(function(state)
      state.sidebar:open()
      state.chdir("/tmp/other-project")
      state.sidebar:new_chat()
      ...
      state.exits[1](0)
      eq("/tmp/other-project", state.starts[2].cwd)
      local command = state.starts[2].command
      truthy(vim.tbl_contains(command, "--session-id"))
      eq(false, vim.tbl_contains(command, "session-id"))
      eq(false, vim.tbl_contains(command, "--session"))
      eq({}, state.sends)
      truthy(state.sidebar:open())
      eq(2, #state.starts)
    end, { defer_exit = true })
  ...
  it("a new chat without an existing process starts immediately without confirmation", function()
    project_fixture(function(state)
      truthy(state.sidebar:new_chat())
      eq(1, #state.starts)
      eq("/tmp/project", state.starts[1].cwd)
      eq(false, vim.tbl_contains(state.starts[1].command, "session-id"))
      eq({}, state.choices)
    end)
  end)

  it("a new chat confirmation cannot change projects or restart after shutdown", function()
    project_fixture(function(state)
      state.sidebar:open()
      state.sidebar:new_chat()
      state.chdir("/tmp/other-project")
      ...
      truthy(state.sidebar:new_chat())
      state.sidebar:shutdown()
      state.choices[2].choose("Start new chat")
      eq(1, #state.starts)
      eq({ 1 }, state.stops)
    end)
  end)

  it("a picker startup error does not leave chat actions locked", function()
    project_fixture(function(state)
      state.sidebar:open()
      select_ui.open = function() error("picker startup failed") end
      state.sidebar:new_chat()
      truthy(state.notices[1]:find("picker startup failed", 1, true))
      truthy(state.sidebar:open())
      eq(1, #state.starts)
      eq({}, state.stops)
    end)
...
describe("vimgentic.chat.terminal", function()
  ...
  it("sends the selected thinking level with a live chat model change", function()
    local sidebar, _, _, sends = fixture()
    sidebar:open()
    sidebar:set_model("other/new-model:high")
    eq({ { job_id = 1, text = "/model other/new-model:high\r" } }, sends)
    cleanup(sidebar)
  end)
